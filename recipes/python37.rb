...
set :virtualenv_name, fetch(:virtualenv_name, "venv37")
set :shared_children, shared_children + [virtualenv_name]
set :virtualenv_python_binary, fetch(:virtualenv_python_binary, "python3.7")
...
namespace :deploy do
  ...
  task :create_virtualenv, :roles => :app do
    run "python3.7 -m venv '#{virtualenv_path}'"
  end
